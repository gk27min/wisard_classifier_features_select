...
class WisardClassifier:
    ...
    def set_train(self, train, labels):
        self.ds_train = wsd.DataSet(train, labels)
        
    def set_test(self, test, labels):
        self.ds_test = wsd.DataSet(test, labels)
    ...
    def fit(self, X, y):
        self.ds_train = wsd.DataSet(X, y)
        self.model.train(self.ds_train)
    
    def score(self, X, y):
        self.ds_test = wsd.DataSet(X, y)
...
class SVMClassifier:
    def __init__(self, kernel='rbf', C=1.0, gamma='scale'):
        self.model = svc.SVC(kernel=kernel, C=C, gamma=gamma)
